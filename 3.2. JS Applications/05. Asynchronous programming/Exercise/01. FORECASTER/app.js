  ...
  function getWeather() {
    ...
    fetch(`https://judgetests.firebaseio.com/locations.json`)
      ...
      .then(x => {
        ...
        if (foundTown) {
          fetch(
            `https://judgetests.firebaseio.com/forecast/today/${foundTown.code}.json`
          )
            .then(x => x.json())
            .then(data => {
              ...
              conditionSymbol.textContent = checkWeatherCondition(
              ...
              secondLine.textContent = `${currentForecast["low"]}°/${
          ...
          fetch(
            `https://judgetests.firebaseio.com/forecast/upcoming/${foundTown.code}/forecast.json`
          )
            .then(x => x.json())
            .then(data => {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                firstLine.textContent = checkWeatherCondition(
